...
export function useCollection<T = any>(
  ...
  const fetchData = useCallback(async (isManualRefetch = false) => {
    ...
    // For manual refetch, we bypass the snapshot listener and just get the data once.
    // This is useful for clearing client-side cache issues.
    if (isManualRefetch) {
      try {
      ...
      return; // Stop here for manual refetch
    }
    ...
    const unsubscribe = onSnapshot(
      ...
      (error: FirestoreError) => {
        ...
        const contextualError = new FirestoreContextualError({
          operation: 'list',
          path,
        }, error);
